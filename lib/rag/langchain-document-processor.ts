 ...
 */
export type LangChainSupportedFileType = 'pdf' | 'txt' | 'md' | 'docx';
...
export async function splitDocumentsWithLangChain(
  ...
  return withLangChainErrorHandling('splitDocuments', async () => {
    return withLangChainTiming('splitDocuments', async () => {
      ...
      console.log(`[LangChain] Splitting ${documents.length} documents with chunk size ${config.chunkSize}`);
...
export async function generateEmbeddingsForDocuments(
  ...
  return withLangChainErrorHandling('generateEmbeddings', async () => {
    return withLangChainTiming('generateEmbeddings', async () => {
      ...
      console.log(`[LangChain] Generating embeddings for ${documents.length} documents`);
      ...
      for (let i = 0; i < texts.length; i += batchSize) {
        ...
        console.log(`[LangChain] Processing embedding batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(texts.length / batchSize)}`);
...
export async function processDocumentWithLangChain(
  ...
  return withLangChainErrorHandling('processDocument', async () => {
    return withLangChainTiming('processDocument', async () => {
      ...
      const splitDocuments = await splitDocumentsWithLangChain(loadedDocuments, config);
      ...
      console.log(`[LangChain] Document processing completed in ${processingTime}ms`);
 ...
 */
export function validateLangChainFileType(fileName: string): LangChainSupportedFileType | null {
...
export async function compareLangChainWithLegacy(
  ...
  const { processDocument: legacyProcessDocument } = await import('./retriever');
  ...
  const comparison = {
    chunkCountDiff: langchainResult.metadata.chunkCount - legacyResult.chunks.length,
    avgChunkSizeDiff: langchainResult.metadata.avgChunkSize - (legacyResult.chunks.reduce((sum: number, chunk: any) => sum + chunk.content.length, 0) / legacyResult.chunks.length),
    processingTimeDiff: langchainResult.metadata.processingTime - (Date.now() - startTime),
  };
