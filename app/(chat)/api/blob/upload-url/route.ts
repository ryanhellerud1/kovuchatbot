    ...
    const jsonResponse = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        ...
        if (size && size > maxSize) {
          throw new Error(`File too large. Maximum size is 15MB. Your file is ${(size / 1024 / 1024).toFixed(1)}MB.`);
        }
        ...
        const nameWithoutExt = filename ? filename.substring(0, filename.lastIndexOf('.')) : 'file';
